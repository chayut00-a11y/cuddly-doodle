...
export async function POST(req) {
  try {
    ...
    if (user) {
      ...
      // เซ็ต Cookie เพื่อให้ Middleware สามารถตรวจสอบสิทธิ์ก่อนเข้าหน้า /user ได้
      response.cookies.set('isLoggedIn', 'true', {
        path: '/',
        httpOnly: true, // ป้องกันการถูกขโมย Cookie ผ่าน JavaScript (XSS)
        secure: process.env.NODE_ENV === 'production',
        maxAge: 3600, // ให้มีอายุ 1 ชั่วโมง
      });
